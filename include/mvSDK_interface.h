...
typedef struct
{
	int edge_thres;      /* edge threshold, 80 */
	float max_whrate;    /* edge component, w/h rate: 2.5f,if not use set to -1*/
	float min_area;      /* edge component: area,if not use set to -1*/
	int bin_thres;       /* binary threshold(0, 255),if not use set to -1 */
	int accu;            /* accumurator, default: 20 */
	int min_radius;      /* min radius, default: 0, if not use set to 0 */
	int max_radius;      /* max radius, default: 0, if not use set to 0 */
	int kernel;          /* smooth kernel: 3, 5, 7,9, 11 */
}detParam;
